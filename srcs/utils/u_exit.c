...
int	c_error(t_errors errors)
{
	if(errors == CHECK_ARG_NUMBER)
		return(ft_putstr_fd(RED"Incorrect format: try ./philolophers 5 800 200 200 [7]\n"RST));
		...
		return(ft_putstr_fd(RED"Wrong opcode for mutex_handle: use <LOCK> <UNLOCK> <INIT> <DESTROY>\n"RST));
	else if(errors == WRONG_THREAD_OP)
		return(ft_putstr_fd(RED"Wrong opcode for thread_handle: use <CREATE> <JOIN> <DETACH>>\n"RST));
	else if(errors == FAILED_GET_TIME)
		return(ft_putstr_fd(RED"Get time of day failed\n"RST));
	else if(errors == WRONG_TIME_OP)
		return(ft_putstr_fd(RED"Wrong opcode for timehandle:use <MILLISECOND> <MICROSECOND> <SECONDS>\n"RST));
	return(0);
}