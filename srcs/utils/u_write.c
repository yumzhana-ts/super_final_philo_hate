		...
		long elapsed)
{
	if (is_error(simulation_finished(philo->table)))
		return (1);
	if (TAKE_FIRST_FORK == status && !is_true(simulation_finished(philo->table)))
		printf(W"%6ld"RST" %d has taken the 1° fork 🍽"
			"\t\t\tn°"B"[🍴 %d 🍴]\n"RST, elapsed, philo->id,
			philo->first_fork->fork_id);
	else if (TAKE_SECOND_FORK == status && !is_true(simulation_finished(philo->table)))
...
int   write_status(t_philo_status status, t_philo *philo, bool debug)
{
	...
	else
	{
		if (is_error(simulation_finished(philo->table)))
			return (1);
		if ((status == TAKE_FIRST_FORK || status == TAKE_SECOND_FORK) && !is_true(simulation_finished(philo->table)))
